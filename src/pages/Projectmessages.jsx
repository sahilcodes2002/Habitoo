...
export function Projectmessages() {
  ...
  const fetchMessages = async () => {
    try {
        //console.log(userId);
      const response = await axios.get(`https://honoprisma.codessahil.workers.dev/getProjectmessages/${userId}`, {
        headers: {
  ...
  const sendMessage = async () => {
    if (newMessage.trim() !== '' && !busy) {
      try {
        setBusy(true);
        const response = await axios.post(`https://honoprisma.codessahil.workers.dev/createprojectmessage/${userId}`, 
          {
            content: newMessage,
          }, 
          {
      ...
      <div className='fixed flex w-full'>
        <div className='bg-gray-200 mb-4 p-4 flex flex-col justify-center'>
          <div className='flex justify-center'>
            <button onClick={() => navigate(`/project/${userId}`)} className="text-gray-600 hover:text-gray-800 focus:outline-none">
